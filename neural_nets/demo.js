const VISUALIZER_CONFIG = {
  weightUrl: "./weights/mlp_weights.json",
...
const MNIST_SAMPLE_MANIFEST_URL = "./data/mnist-test-manifest.json";

document.addEventListener("DOMContentLoaded", () => {
  initializeVisualizer().catch((error) => {
    console.error(error);
    renderErrorMessage("The visualization could not be initialized. See the console for details.");
  });
});
...
async function loadMnistTestSamples(manifestPath = MNIST_SAMPLE_MANIFEST_URL) {
  ...
  if (numSamples <= 0) {
    if (sampleSize > 0) {
      ...
      if (inferredSamples <= 0) {
        throw new Error("Could not infer a sample size from the MNIST image data.");
      }
      if (labelBytes.length !== inferredSamples) {
        throw new Error("Number of labels does not match the inferred number of samples.");
      }
    } else {
  ...
  if (imageBytes.length !== totalSamples * sampleSize) {
    throw new Error("MNIST image data length does not match the expected size.");
  }
  if (labelBytes.length !== totalSamples) {
    throw new Error("MNIST label data length does not match the expected size.");
  }
...
async function setupMnistSampleButtons({ digitCanvas, onSampleApplied, manifestPath } = {}) {
  ...
  for (let digit = 0; digit < 10; digit += 1) {
    ...
    button.setAttribute("aria-label", `Load a random ${digit}`);
...
function setupTimelineSlider(timelineSnapshots, options = {}) {
  ...
  slider.addEventListener("input", (event) => {
    ...
    setActiveIndex(nextIndex, { emit: true }).catch((error) => {
      console.error("Error while updating the snapshot:", error);
    });
  });
